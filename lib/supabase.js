...
export const recordItemOil = async (userId, itemKey) => {
  ...
  const { data: existing } = await supabase
    ...
    .eq("item_key", itemKey)
    .maybeSingle();
  const nextCount = Number(existing?.oil_count || 0) + 1;
  const payload = { user_id: userId, item_key: itemKey, oil_count: nextCount, last_oiled_at: now, updated_at: now };
  await supabase.from("user_data").upsert(payload, { onConflict: "user_id,item_key" });
  return payload;
};
...
export const createFolder = async (userId, name, options = {}) => {
  ...
  await supabase.from("vault_folders").insert({
    ...
    parent_folder: options.parent_folder || null,
  });
  return cleaned;
};
...
export const recordFolderView = async (userId, name) => {
  ...
  const { data, error } = await supabase
    .from("vault_folders")
    .update({ view_count: nextCount, last_viewed_at: now, updated_at: now })
    .eq("user_id", userId)
    .ilike("name", cleaned)
    .select("*")
    .maybeSingle();
  ...
  await supabase.from("vault_items").upsert(
    {
      ...
      cover_position_x: Number.isFinite(Number(item.cover_position_x)) ? Number(item.cover_position_x) : 50,
      cover_position_y: Number.isFinite(Number(item.cover_position_y)) ? Number(item.cover_position_y) : 50,
      type: item.type || "link",
