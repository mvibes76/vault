...
export const deleteFolder = async (userId, name) => {
  ...
  await supabase.from("user_data").update({ folder: null, updated_at: new Date().toISOString() }).eq("user_id", userId).eq("folder", name);
  await supabase.from("vault_items").update({ folder: null, updated_at: new Date().toISOString() }).eq("user_id", userId).eq("folder", name);
};
