...
const HLS_RE      = /\.(m3u8)(\?|$)/i;
const IMG_RE      = /\.(jpg|jpeg|png|gif|webp|avif|bmp)(\?|$)/i;
const PDF_RE      = /\.(pdf)(\?|$)/i;
...
export const SOURCES = [
  ...
  },
  {
    ...
    embed: { kind: "drive", build: (id) => `https://drive.google.com/file/d/${id}/preview`, direct: (id) => `https://drive.google.com/uc?export=download&id=${id}`, sizing: "drive" },
  },
  {
    id: "tiktok",
    name: "TikTok",
    ...
    thumb: () => null,
    embed: { kind: "iframe", build: (id) => `https://www.tiktok.com/embed/v2/${id}`, sizing: "portrait" },
  },
  {
    id: "twitter",
    name: "X / Twitter",
  ...
  },
  {
    ...
    embed: { kind: "iframe", build: (_id, url) =>
      `https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(url)}&show_text=false&autoplay=true`, sizing: "tall" },
  },
  {
    id: "instagram",
    name: "Instagram",
    ...
    thumb: () => null,
    embed: { kind: "iframe", build: (id) => `https://www.instagram.com/p/${id}/embed/`, sizing: "portrait" },
  },
  {
    id: "reddit",
    name: "Reddit",
...

  {
    ...
    name: "PDF",
    color: "#E11D48",
    match: (u) => PDF_RE.test(u),
    id_of: (u) => u,
    thumb: () => null,
    embed: { kind: "pdf", src: (url) => url },
  },
  {
    id: "image",
    name: "Image",
...
export function getThumbCandidates(url) {
  ...
  const thumb = s.thumb(id, url);
  return thumb ? [thumb] : [];
}

export function getThumb(url) {
  return getThumbCandidates(url)[0] || null;
}
...
export function getEmbed(url, opts = {}) {
  ...
  if (k === "iframe") return { kind: "iframe", src: s.embed.build(id, url, opts), sizing: s.embed.sizing || "wide", source: s };
  ...
  if (k === "pdf")    return { kind: "pdf",    src: s.embed.src(url), source: s };
  if (k === "drive")  return { kind: "drive",  src: s.embed.direct(id), preview: s.embed.build(id, url, opts), sizing: s.embed.sizing || "drive", source: s };
